        ...
        *********************************************************************************************************************************************************** -->
		...
		<section id="main-content">
			<section class="wrapper">
				<h3>
					<i class="fa fa-angle-right"></i> 상품관리
				</h3>
				<div class="row">
					...
					<div class="content-panel">
						<div class="adv-table">
							<table cellpadding="0" cellspacing="0" border="0"
								...
								<tbody>
									<c:forEach var="vo" items="${prod_list}">
										<tr>
											<td><input name="RowCheck" type="checkbox" value="${vo.prodNumber}"></td>
											<td><a href="/admin/prodView?prodNumber=${vo.prodNumber}">${vo.prodNumber}</a></td>
											<td>${vo.prodName}</td>
											<td>${vo.prodRecievingDate}</td>
											<td>${vo.prodStock}</td>
											<td>${vo.prodCategory}</td>
											<td>${vo.prodPrice}</td>
											<td>${vo.prodServings}</td>
											<td>${vo.prodCookingTime}</td>
											<td>${vo.prodPrepareTime}</td>
											<td>${vo.prodComment}</td>
										</tr>
									</c:forEach>
								</tbody>
							</table>
							<table>
								<tr>
									<td colspan="2"><input type="button" value="선택 삭제"
										...
										&nbsp;&nbsp;<a href="/admin/prodAdd"><button type="button"
												class="btn btn-default">상품 등록 하기</button></a></td>
								</tr>
							</table>
						</div>
					</div>
		...
		function deleteValue() {
			...
			if (valueArr.length == 0) {
				...
				$.ajax({
					...
					traditional : true,
					data : {
						valueArr : valueArr					// 보내고자 하는 date 변수 
					},
